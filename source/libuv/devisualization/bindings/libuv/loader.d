 ...
 */
module devisualization.bindings.libuv.loader;
	...
	private {
		...
		version(OSX) {
			...
			version(ARM) {
			    ...
			    static string NugetFile = "runtimes/linux-x64/native/libuv.so";
			    static string[] ToLoadFiles = ["libuv.so", "/usr/lib/x86_64-linux-gnu/libuv.so.1"];
			} else {
			    static string[] ToLoadFiles = ["libuv.so"];
			}
		} else version(Windows) {
			static string[] ToLoadFiles = ["libuv.dll"];
			enum ToLoadName = "libuv.dll";
			version(ARM) {
	...
	this(string file) {
		...
		try {
			...
			static if (__traits(compiles, {string s = NugetFile; })) {
				...
				tempFile = buildPath(tempFile, ToLoadName);
				if (exists(tempFile)) {
				} else {
	...
	private {
		void loadSymbols() {
            ...
            with(libuv) {
                ...
                uv_translate_sys_error = cast(typeof(uv_translate_sys_error))loader.loadSymbol("uv_translate_sys_error", false);
